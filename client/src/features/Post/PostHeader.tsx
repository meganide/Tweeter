...
import { httpEditPost } from '../../hooks/requests';
...
function PostHeader(props: IProps) {
  ...
  const editPostMutation = useCustomMutation(httpEditPost, 'followersPosts');

  async function handleSaveEdit() {
    setLoading(true);
    await editPostMutation.mutate({
      postId: postData.id,
      content: editPostQuery,
  ...
  return (
    <header className="flex w-full items-center gap-6">
      ...
      >
        <h2 className="text-neutral-900 dark:text-neutral-300">
      ...
      {currentUser?.id === postData.authorId && editPost ? (
        ...
        currentUser?.id === postData.authorId && (
          <PostDeleteButtons
            deleteProps={{ loading, setLoading, postData, handleCancel }}
          />
        )
      )}
    </header>
